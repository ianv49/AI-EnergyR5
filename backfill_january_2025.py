#!/usr/bin/env python3
"""
Backfill January 2025 data for all sources
"""
...
def generate_weather_data(timestamp, source):
    """Generate realistic weather data for Manila - January 2025"""
    hour = timestamp.hour
    month = timestamp.month  # January = 1
    
    # Temperature (January is coolest - dry season)
    if month in [12, 1, 2]:
        base_temp = 26.0
    elif month in [3, 4, 5]:
        base_temp = 30.0
    ...
    # Wind speed
    base_wind = 3.8  # Stronger in dry season
    if 10 <= hour < 16:
    ...
    # Cloudiness (January is clearest - dry season)
    if 6 <= hour < 18:
        cloudiness = random.uniform(8, 40)
    else:
        cloudiness = random.uniform(5, 20)
    
    # UV Index
    if 6 <= hour < 18:
        uv_index = max(0, min(8, (hour - 6) * 0.8 + random.uniform(-1, 1)))
    else:
        uv_index = 0
    ...
    if 6 <= hour < 18:
        ...
        base_irradiance = 730 * peak_factor
...
def backfill_january_2025():
    ...
    logger.info("="*60)
    logger.info("JANUARY 2025 BACKFILL")
    logger.info("="*60)
