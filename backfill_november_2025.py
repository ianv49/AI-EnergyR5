#!/usr/bin/env python3
"""
Backfill November 2025 data for all sources
Reuses functions from backfill_december_2025.py
"""
...
def generate_weather_data(timestamp, source):
    """Generate realistic weather data for Manila - November 2025"""
    hour = timestamp.hour
    month = timestamp.month  # November = 11
    
    # Temperature based on month and time (November is transitional)
    if month in [11, 12, 1, 2]:
        base_temp = 27.0  # Slightly cooler in November
    elif month in [3, 4, 5]:
        base_temp = 30.0
    else:
        base_temp = 28.0
    ...
    # Humidity (November is transitioning to drier season)
    base_humidity = 72.0
    if 0 <= hour < 6:
    ...
    # Cloudiness (November is less cloudy)
    if 6 <= hour < 18:
        cloudiness = random.uniform(30, 70)
    else:
        cloudiness = random.uniform(15, 45)
    
    # UV Index
    if 6 <= hour < 18:
        uv_index = max(0, min(9, (hour - 6) * 1.0 + random.uniform(-1, 1)))
    else:
        uv_index = 0
...
def backfill_source(source, start_date, end_date):
    """Backfill data for a specific source for November 2025"""
    conn = get_connection()
    ...
    logger.info(f"Existing {source} records for Nov 2025: {len(existing)}")
...
def backfill_november_2025():
    """Main function to backfill all sources for November 2025"""
    # November 2025 date range (30 days)
    start_date = datetime(2025, 11, 1, 0, 0, 0)
    end_date = datetime(2025, 11, 30, 23, 0, 0)
    
    logger.info("="*60)
    logger.info("NOVEMBER 2025 BACKFILL")
    logger.info("="*60)
    ...
    # Summary
    logger.info("\n" + "="*60)
    logger.info("NOVEMBER 2025 BACKFILL COMPLETE")
    logger.info("="*60)
...
def verify_coverage():
    """Verify November 2025 data coverage"""
    conn = get_connection()
    
    with conn.cursor() as cur:
        # Check counts per source for November 2025
        cur.execute("""
            SELECT source, COUNT(*) as cnt
            FROM sensor_data 
            WHERE timestamp >= '2025-11-01' AND timestamp < '2025-12-01'
            GROUP BY source
            ORDER BY source
        """)
