#!/usr/bin/env python3
"""
NASA POWER Backfill for September 2025 Only
...
def backfill_september_2025():
    """Backfill NASA POWER data for September 2025"""
    
    start_date = datetime(2025, 9, 1)
    end_date = datetime(2025, 10, 1)
    
    logger.info("="*60)
    logger.info("NASA POWER BACKFILL - SEPTEMBER 2025")
    logger.info("="*60)
    logger.info(f"Date range: {start_date.date()} to {end_date.date()}")
    logger.info(f"Expected: 30 days x 24 hours = 720 hours")
    
    deleted = delete_nasa_data_for_range(start_date, end_date)
    logger.info(f"Deleted {deleted} existing records for September 2025")
    ...
    while current < end_date:
        ...
        if result:
            ...
            base = 480 + 100 * (1 - abs(180 - day_of_year) / 180)
    ...
    logger.info("\n" + "="*60)
    logger.info("NASA POWER SEPTEMBER 2025 BACKFILL COMPLETE")
    logger.info("="*60)
...
def verify_coverage():
    ...
    with conn.cursor() as cur:
        ...
        print("\n" + "="*60)
        print("SEPTEMBER 2025 DATA SUMMARY")
        print("="*60)
        ...
        cur.execute("""
            ...
            FROM sensor_data 
            WHERE source = 'nasa_power' AND timestamp >= '2025-09-01' AND timestamp < '2025-10-01'
        """)
        row = cur.fetchone()
        print(f"\n  NASA POWER Details:")
