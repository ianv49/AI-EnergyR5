#!/usr/bin/env python3
"""
Backfill March 2025 data for all sources
...
def fetch_nasa_solar_data(target_date):
    ...
    try:
        ...
        return 5500  # Typical daily average W/m² for March
...
def generate_weather_data(timestamp, source):
    """Generate realistic weather data for Manila - March 2025"""
    hour = timestamp.hour
    month = timestamp.month  # March = 3
    
    # Temperature (March is warm - transitioning to summer)
    if month in [3, 4, 5]:
        base_temp = 29.5
    else:
        base_temp = 27.5
    ...
    # Cloudiness
    if 6 <= hour < 18:
        cloudiness = 35  # More clouds in March
    else:
        cloudiness = 20
    
    # UV Index
    if 6 <= hour < 18:
        uv_index = max(0, min(10, (hour - 6) * 1.0))
    else:
        uv_index = 0
...
def backfill_source(source, start_date, end_date):
    ...
    with conn.cursor() as cur:
        ...
        conn.commit()
        logger.info(f"Deleted existing {source} records for Mar 2025")
    ...
    logger.info(f"Starting {source} backfill for Mar 2025...")
...
def backfill_march_2025():
    ...
    logger.info("="*60)
    logger.info("MARCH 2025 BACKFILL (v2 - NASA POWER API)")
    logger.info("="*60)
    ...
    logger.info("\n" + "="*60)
    logger.info("MARCH 2025 BACKFILL COMPLETE")
    logger.info("="*60)
