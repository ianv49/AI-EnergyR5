#!/usr/bin/env python3
"""
Backfill April 2025 data for all sources
"""
...
def generate_weather_data(timestamp, source):
    """Generate realistic weather data for Manila - April 2025"""
    hour = timestamp.hour
    month = timestamp.month  # April = 4
    
    # Temperature (April is hot - summer)
    if month in [3, 4, 5]:
        base_temp = 30.0
    else:
        base_temp = 28.0
    ...
    if 6 <= hour < 18:
        ...
        cloudiness = random.uniform(10, 35)
    ...
    if 6 <= hour < 18:
        ...
        base_irradiance = 820 * peak_factor
        cloud_adjustment = (100 - cloudiness) / 100
        uv_adjustment = uv_index * 24 if uv_index > 0 else 0
...
def backfill_source(source, start_date, end_date):
    ...
    logger.info(f"Existing {source} records for Apr 2025: {len(existing)}")
...
def backfill_april_2025():
    ...
    end_date = datetime(2025, 4, 30, 23, 0, 0)
    
    logger.info("="*60)
    logger.info("APRIL 2025 BACKFILL")
    logger.info("="*60)
    ...
    logger.info("\n" + "="*60)
    logger.info("APRIL 2025 BACKFILL COMPLETE")
    logger.info("="*60)
