...
def generate_weather_data(timestamp, source):
    ...
    # Cloudiness
    if 6 <= hour < 18:
        cloudiness = random.uniform(15, 55)
    else:
        cloudiness = random.uniform(10, 30)
    
    # UV Index
    if 6 <= hour < 18:
        uv_index = max(0, min(10, (hour - 6) * 1.0 + random.uniform(-1, 1)))
    else:
        uv_index = 0
    ...
    if 6 <= hour < 18:
        ...
        base_irradiance = 780 * peak_factor
...
def backfill_source(source, start_date, end_date):
    ...
    logger.info(f"Existing {source} records for Mar 2025: {len(existing)}")
...
def backfill_march_2025():
    ...
    logger.info("="*60)
    logger.info("MARCH 2025 BACKFILL")
    logger.info("="*60)
