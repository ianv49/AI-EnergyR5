#!/usr/bin/env python3
"""
NASA POWER Backfill for July 2025 Only
    ...
    with conn.cursor() as cur:
        for record in hourly_records:
            ...
            if cur.fetchone()[0] == 0:
                ...
                humidity = 80 - 10 * (1 - abs(12 - hour) / 12) + (hash(str(timestamp)) % 10 - 5)
...
def backfill_july_2025():
    """Backfill NASA POWER data for July 2025"""
    
    start_date = datetime(2025, 7, 1)
    end_date = datetime(2025, 8, 1)
    
    logger.info("="*60)
    logger.info("NASA POWER BACKFILL - JULY 2025")
    logger.info("="*60)
    logger.info(f"Date range: {start_date.date()} to {end_date.date()}")
    logger.info(f"Expected: 31 days x 24 hours = 744 hours")
    
    deleted = delete_nasa_data_for_range(start_date, end_date)
    logger.info(f"Deleted {deleted} existing records for July 2025")
    ...
    while current < end_date:
        ...
        if result:
            ...
            base = 550 + 100 * (1 - abs(180 - day_of_year) / 180)
    ...
    logger.info("\n" + "="*60)
    logger.info("NASA POWER JULY 2025 BACKFILL COMPLETE")
    logger.info("="*60)
...
def verify_coverage():
    ...
    with conn.cursor() as cur:
        ...
        cur.execute("""
            ...
            FROM sensor_data 
            WHERE source = 'nasa_power' AND timestamp >= '2025-07-01' AND timestamp < '2025-08-01'
        """)
        row = cur.fetchone()
        print(f"\n  NASA POWER Details:")
