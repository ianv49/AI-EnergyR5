#!/usr/bin/env python3
"""
Backfill July 2025 data for all sources
Reuses functions from backfill_september_2025.py
"""
...
def generate_weather_data(timestamp, source):
    """Generate realistic weather data for Manila - July 2025"""
    hour = timestamp.hour
    month = timestamp.month  # July = 7
    
    # Temperature based on month and time (July is summer in Manila)
    if month in [3, 4, 5]:
        base_temp = 30.0
    elif month in [6, 7, 8]:
        base_temp = 29.5  # Hot in July
    else:
        base_temp = 27.5
    ...
    # Humidity (July is humid - monsoon season)
    base_humidity = 80.0
    if 0 <= hour < 6:
    ...
    # Cloudiness (July is cloudy - wet season)
    if 6 <= hour < 18:
        cloudiness = random.uniform(50, 90)
    else:
...
def backfill_source(source, start_date, end_date):
    """Backfill data for a specific source for July 2025"""
    conn = get_connection()
    ...
    logger.info(f"Existing {source} records for Jul 2025: {len(existing)}")
...
def backfill_july_2025():
    """Main function to backfill all sources for July 2025"""
    # July 2025 date range (31 days)
    start_date = datetime(2025, 7, 1, 0, 0, 0)
    end_date = datetime(2025, 7, 31, 23, 0, 0)
    
    logger.info("="*60)
    logger.info("JULY 2025 BACKFILL")
    logger.info("="*60)
    ...
    # Summary
    logger.info("\n" + "="*60)
    logger.info("JULY 2025 BACKFILL COMPLETE")
    logger.info("="*60)
...
def verify_coverage():
    """Verify July 2025 data coverage"""
    conn = get_connection()
