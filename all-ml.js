// all-ml.js - Unified ML Data Injection (All Sources)
// Fetches data/all-ml.txt, extracts Data-A (predict) & Data-B (actual), injects into all-ml.html.
// Displays prediction vs actual comparison across all combined weather sources.

async function fetchAndParseAllMLData() {
  console.log('All-ML: Fetching data/all-ml.txt...');
  
  try {
    const response = await fetch('data/all-ml.txt');
    ...
    console.log('✅ Data successfully parsed - ready for chart update');
    
    // Parse metrics
    const maeMatch = metricsText.match(/MAE.*?:\s*([\d.-]+)/i);
    ...
    console.log('✅ Data validated & metrics injected');
    ...
    console.error('All-ML ERROR:', error.message);
...
function updateChartsWithData() {
  ...
  if (windChart) {
    ...
    console.warn('⚠️ Wind chart not found');
  }
  ...
  if (solarChart) {
    ...
    console.warn('⚠️ Solar chart not found');
  }
}
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', () => {
    ...
    setTimeout(fetchAndParseAllMLData, 100);
  });
} else {
