#!/usr/bin/env python3
"""
Backfill June 2025 data for all sources
Reuses functions from backfill_july_2025.py
"""
...
def generate_weather_data(timestamp, source):
    """Generate realistic weather data for Manila - June 2025"""
    hour = timestamp.hour
    month = timestamp.month  # June = 6
    
    # Temperature based on month and time (June is start of rainy season)
    if month in [3, 4, 5]:
        base_temp = 30.0
    elif month in [6, 7, 8]:
        base_temp = 29.0  # Warm in June
    else:
        base_temp = 27.5
    ...
    # Humidity (June is humid - monsoon starting)
    base_humidity = 79.0
    if 0 <= hour < 6:
        humidity = base_humidity + 12
    elif 10 <= hour < 15:
        humidity = base_humidity - 14
    else:
    ...
    base_wind = 2.4
    if 10 <= hour < 16:
        wind_speed = base_wind + 1.4
    elif 0 <= hour < 6:
        wind_speed = base_wind - 0.4
    else:
        wind_speed = base_wind + 0.3
    
    wind_speed = max(0.5, wind_speed + random.uniform(-1, 1))
    
    # Cloudiness (June is cloudy - monsoon season)
    if 6 <= hour < 18:
        cloudiness = random.uniform(45, 85)
    else:
        cloudiness = random.uniform(25, 55)
    
    # UV Index
    if 6 <= hour < 18:
        uv_index = max(0, min(10, (hour - 6) * 0.9 + random.uniform(-1, 1)))
    else:
        uv_index = 0
    ...
    if 6 <= hour < 18:
        ...
        base_irradiance = 710 * peak_factor
...
def backfill_source(source, start_date, end_date):
    """Backfill data for a specific source for June 2025"""
    conn = get_connection()
    ...
    logger.info(f"Existing {source} records for Jun 2025: {len(existing)}")
...
def backfill_june_2025():
    """Main function to backfill all sources for June 2025"""
    # June 2025 date range (30 days)
    start_date = datetime(2025, 6, 1, 0, 0, 0)
    end_date = datetime(2025, 6, 30, 23, 0, 0)
    
    logger.info("="*60)
    logger.info("JUNE 2025 BACKFILL")
    logger.info("="*60)
    ...
    # Summary
    logger.info("\n" + "="*60)
    logger.info("JUNE 2025 BACKFILL COMPLETE")
    logger.info("="*60)
...
def verify_coverage():
    """Verify June 2025 data coverage"""
    conn = get_connection()
