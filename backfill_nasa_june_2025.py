#!/usr/bin/env python3
"""
NASA POWER Backfill for June 2025 Only
    ...
    with conn.cursor() as cur:
        for record in hourly_records:
            ...
            if cur.fetchone()[0] == 0:
                ...
                temperature = 29 + 4 * (1 - abs(12 - hour) / 12) + (hash(str(timestamp)) % 10 - 5) / 5
                humidity = 78 - 10 * (1 - abs(12 - hour) / 12) + (hash(str(timestamp)) % 10 - 5)
...
def backfill_june_2025():
    """Backfill NASA POWER data for June 2025"""
    
    start_date = datetime(2025, 6, 1)
    end_date = datetime(2025, 7, 1)
    
    logger.info("="*60)
    logger.info("NASA POWER BACKFILL - JUNE 2025")
    logger.info("="*60)
    logger.info(f"Date range: {start_date.date()} to {end_date.date()}")
    logger.info(f"Expected: 30 days x 24 hours = 720 hours")
    
    deleted = delete_nasa_data_for_range(start_date, end_date)
    logger.info(f"Deleted {deleted} existing records for June 2025")
    ...
    logger.info("\n" + "="*60)
    logger.info("NASA POWER JUNE 2025 BACKFILL COMPLETE")
    logger.info("="*60)
...
def verify_coverage():
    ...
    with conn.cursor() as cur:
        ...
        cur.execute("""
            ...
            FROM sensor_data 
            WHERE source = 'nasa_power' AND timestamp >= '2025-06-01' AND timestamp < '2025-07-01'
        """)
        row = cur.fetchone()
        print(f"\n  NASA POWER Details:")
