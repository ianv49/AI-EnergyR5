#!/usr/bin/env python3
"""
Backfill August 2025 data for all sources
Reuses functions from backfill_october_2025.py
"""
...
def generate_weather_data(timestamp, source):
    """Generate realistic weather data for Manila - August 2025"""
    hour = timestamp.hour
    month = timestamp.month  # August = 8
    
    # Temperature based on month and time (August is peak summer in Manila)
    if month in [3, 4, 5]:
        base_temp = 30.0  # Hot in summer
    elif month in [6, 7, 8, 9]:
        base_temp = 29.0  # Still hot in August (wet season)
    else:
        base_temp = 27.5
    ...
    # Humidity (August is humid - rainy season)
    base_humidity = 82.0  # High humidity in August
    if 0 <= hour < 6:
    ...
    # Wind speed
    base_wind = 2.5  # Light wind in August
    if 10 <= hour < 16:
    ...
    # Cloudiness (August is cloudy - monsoon season)
    if 6 <= hour < 18:
        cloudiness = random.uniform(50, 90)  # Very cloudy in August
    else:
        cloudiness = random.uniform(30, 60)
    
    # UV Index
    if 6 <= hour < 18:
        uv_index = max(0, min(10, (hour - 6) * 0.85 + random.uniform(-1, 1)))
    else:
        uv_index = 0
    ...
    if 6 <= hour < 18:
        ...
        base_irradiance = 680 * peak_factor  # Lower due to clouds/rain
        cloud_adjustment = (100 - cloudiness) / 100
        uv_adjustment = uv_index * 18 if uv_index > 0 else 0
...
def backfill_source(source, start_date, end_date):
    """Backfill data for a specific source for August 2025"""
    conn = get_connection()
    ...
    logger.info(f"Existing {source} records for Aug 2025: {len(existing)}")
...
def backfill_august_2025():
    """Main function to backfill all sources for August 2025"""
    # August 2025 date range (31 days)
    start_date = datetime(2025, 8, 1, 0, 0, 0)
    end_date = datetime(2025, 8, 31, 23, 0, 0)
    
    logger.info("="*60)
    logger.info("AUGUST 2025 BACKFILL")
    logger.info("="*60)
    ...
    # Summary
    logger.info("\n" + "="*60)
    logger.info("AUGUST 2025 BACKFILL COMPLETE")
    logger.info("="*60)
...
def verify_coverage():
    """Verify August 2025 data coverage"""
    conn = get_connection()
    
    with conn.cursor() as cur:
        # Check counts per source for August 2025
        cur.execute("""
