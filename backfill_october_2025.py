#!/usr/bin/env python3
"""
Backfill October 2025 data for all sources
Reuses functions from backfill_november_2025.py
"""
...
def generate_weather_data(timestamp, source):
    """Generate realistic weather data for Manila - October 2025"""
    hour = timestamp.hour
    month = timestamp.month  # October = 10
    
    # Temperature based on month and time (October is still warm, transitioning from wet to dry)
    if month in [10, 11, 12, 1, 2]:
        base_temp = 27.5  # Warm in October
    elif month in [3, 4, 5]:
        base_temp = 30.0
    else:
        base_temp = 28.0
    ...
    # Humidity (October is still humid - end of rainy season)
    base_humidity = 78.0  # Higher humidity in October (wet season ending)
    if 0 <= hour < 6:
    ...
    # Wind speed
    base_wind = 2.8  # Light wind in October
    if 10 <= hour < 16:
    ...
    # Cloudiness (October is still cloudy - monsoon season ending)
    if 6 <= hour < 18:
        cloudiness = random.uniform(45, 85)  # More cloudy in October
    else:
        cloudiness = random.uniform(25, 55)
    ...
    if 6 <= hour < 18:
        ...
        base_irradiance = 720 * peak_factor  # Slightly lower due to clouds
...
def backfill_source(source, start_date, end_date):
    """Backfill data for a specific source for October 2025"""
    conn = get_connection()
    ...
    logger.info(f"Existing {source} records for Oct 2025: {len(existing)}")
...
def backfill_october_2025():
    """Main function to backfill all sources for October 2025"""
    # October 2025 date range (31 days)
    start_date = datetime(2025, 10, 1, 0, 0, 0)
    end_date = datetime(2025, 10, 31, 23, 0, 0)
    
    logger.info("="*60)
    logger.info("OCTOBER 2025 BACKFILL")
    logger.info("="*60)
    ...
    # Summary
    logger.info("\n" + "="*60)
    logger.info("OCTOBER 2025 BACKFILL COMPLETE")
    logger.info("="*60)
...
def verify_coverage():
    """Verify October 2025 data coverage"""
    conn = get_connection()
    
    with conn.cursor() as cur:
        # Check counts per source for October 2025
        cur.execute("""
