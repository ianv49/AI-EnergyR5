...
def main():
    ...
    try:
        with conn.cursor() as cur:
            ...
            # Latest solcast row
            cur.execute("SELECT * FROM sensor_data WHERE source = 'solcast' ORDER BY timestamp DESC LIMIT 1;")
            solcast_row = cur.fetchall()
            if solcast_row:
                if headers is None:
                    headers = [desc[0] for desc in cur.description]
                print("\n🔎 Latest Solcast row:")
                print(tabulate(solcast_row, headers=headers, tablefmt="psql"))
